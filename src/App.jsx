...
async function pedirClima(lat,lon){
  ...
  async function intentar(url){
    try{
      ...
      if(!r.ok){ ultimoError="HTTP "+r.status;
        try{ const er=await r.json(); if(er&&er.reason) ultimoError+=": "+er.reason; }catch(e){}
        return null; }
      const j=await r.json();
      if(!j.daily){ ultimoError="respuesta sin datos diarios"; return null; }
      return j;
    }catch(e){ ultimoError=(e&&e.name==="TimeoutError")?"tiempo de espera agotado (20s)":(e&&e.message)||"error de red"; return null; }
  }
...
async function muestrearCapa(fn, sitios, clave){
  ...
  const vetoAbsoluto=(cuerpoAgua&&cuerpoAgua.enAgua)||(humedal&&humedal.esHumedal)||(protegida&&protegida.protegida)||(extras&&extras.activas&&Object.keys(extras.activas).length>0);
          ...
          {clima&&(<>
            ...
            {vetoAbsoluto&&(
              <div style={{background:"#F0EEE6",borderRadius:8,padding:"12px 14px",fontSize:12.5,color:P.suave,lineHeight:1.5,marginBottom:12}}>
                Este punto <strong>no es plantable</strong> (ver motivo arriba), por lo que no se muestran clima, suelo ni precios: no aplican a un sitio donde no se puede establecer un viñedo.
              </div>
            )}
