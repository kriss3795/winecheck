...
function veredictoGlobal(clima,agua,suelo,humedal,protegida,pendiente,extras){
  if(humedal&&humedal.esHumedal)return "veto";
  if(protegida&&protegida.protegida)return "veto";
  if(extras&&Object.keys(extras).length>0)return "veto";
...
function fraseGlobal(g,clima,agua,w,rec,humedal,protegida,pendiente,extras){
  if(extras&&Object.keys(extras).length>0){const k=Object.keys(extras)[0];const e=extras[k];return "No plantable: el punto está en "+e.tipo.toLowerCase()+(e.nombre?" ("+e.nombre+")":"")+". "+e.detalle;}
...
// Capas de restricción adicionales (todas polígonos nacionales oficiales).
async function pedirRestriccionesExtra(lat,lon){
  const capas=[
    {clave:"glaciar", tipo:"Glaciar", url:"https://rest-sit.mop.gob.cl/arcgis/rest/services/DGA/Inventario_de_Glaciares/MapServer/0", campos:["NOMBRE","Nombre","NOM_GLACIA"], detalle:"Inventario oficial de glaciares (DGA). Zona de hielo permanente: no plantable."},
    {clave:"bosque", tipo:"Bosque nativo", url:"https://ide.minagri.gob.cl/geoserver/conaf/wms", campos:["USO","SUBUSO","TIPO_FOR"], detalle:"Catastro de bosque nativo (CONAF). Talar bosque nativo para plantar está prohibido/regulado por la Ley 20.283.", wms:true},
  ];
  const res={};
  for(const c of capas){
    if(c.wms)continue; // WMS no soporta query directo; se omite por ahora
    const r=await puntoEnCapa(c.url,lat,lon,c.campos);
    if(r.hay){res[c.clave]={tipo:c.tipo,nombre:r.nombre,detalle:c.detalle};}
  }
  return res;
}

// ÁREA PROTEGIDA: parque nacional, reserva, santuario, monumento, RAMSAR, etc.
...
async function pedirPendiente(puntos, centro){
  try{
    ...
    return{ok:true,elevMin:Math.round(emin),elevMax:Math.round(emax),desnivel:Math.round(desnivel),pendiente:pendientePct,estado};
...
export default function App(){
  ...
  async function evaluar(){
    ...
    try{
      const [c,a,suMulti,hum,soil,prot,pend,ext]=await Promise.all([
        pedirClima(la,lo),pedirAgua(la,lo),pedirSuelosMultiples(puntos,[la,lo]),
        pedirHumedal(la,lo),pedirSoilGrids(la,lo),
      ...
      setHumedal(hum);setSg(soil);setProtegida(prot);setPendiente(pend);setExtras(ext||{});setFase("resultado");
    }catch(e){setError(e.message||"error");setFase("resultado");}
  }
  function reiniciar(){setPuntos([]);setFase("mapa");setClima(null);setAgua(null);setSuelo(null);setSuelosMulti([]);setHumedal(null);setSg(null);setProtegida(null);setPendiente(null);setExtras({});setCentro(null);setError(null);setBib(false);}
  ...
  const veredicto=clima?veredictoGlobal(clima,agua,suelo,humedal,protegida,pendiente,extras):"nd";
        ...
        <>
          ...
          {clima&&(<>
            <div style={{background:color,color:"#fff",borderRadius:12,padding:"18px 20px",marginBottom:14}}>
              ...
              <p style={{margin:0,fontSize:14,lineHeight:1.45,opacity:0.96}}>{fraseGlobal(veredicto,clima,agua,w,rec,humedal,protegida,pendiente,extras)}</p>
            </div>
            ...
            {extras&&Object.keys(extras).length>0&&Object.entries(extras).map(([k,e])=>(
            ...
            {!((humedal&&humedal.esHumedal)||(protegida&&protegida.protegida)||(pendiente&&pendiente.ok&&pendiente.estado==="veto")||(extras&&Object.keys(extras).length>0))&&(
            ...
            <Tarjeta titulo="Suelo · estudio agrológico CIREN" estado={suelo?suelo.estado:"nd"}
              ...
              {suelo&&suelo.conectado?(
                <>
                  ...
                  <div style={{fontSize:11.5,color:P.suave,lineHeight:1.4,marginTop:8,fontStyle:"italic"}}>CIREN publica por internet solo la capacidad de uso. Textura, pH y profundidad requieren el informe de suelo o una calicata.</div>
                  {sg&&(sg.ph!=null||sg.clay!=null)&&(
                    <div style={{marginTop:10,paddingTop:10,borderTop:"1px dashed "+P.linea}}>
                      <div style={{fontSize:12,fontWeight:700,color:P.vid,marginBottom:6}}>Complemento físico-químico (SoilGrids):</div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                        {sg.ph!=null&&<Dato valor={sg.ph} etiqueta="pH (agua) 0–5 cm" />}
                        {sg.mo!=null&&<Dato valor={sg.mo} unidad="%" etiqueta="materia orgánica" />}
                        {sg.clay!=null&&<Dato valor={sg.clay} unidad="%" etiqueta="arcilla" />}
                        {sg.sand!=null&&<Dato valor={sg.sand} unidad="%" etiqueta="arena" />}
                      </div>
                      <div style={{fontSize:11,color:"#A39C8E",marginTop:2}}>Fuente: SoilGrids 2.0 (ISRIC), modelo global 250 m. ESTIMACIÓN ORIENTATIVA por interpolación, no medición local (el pH explica ~19-83% de la variación según validación). Para decisión de plantación, confirmar con análisis de laboratorio de una calicata.</div>
                    </div>
                  )}
