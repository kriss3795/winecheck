...
async function pedirClima(lat,lon){
  ...
  const url="https://archive-api.open-meteo.com/v1/archive?latitude="+lat.toFixed(4)+"&longitude="+lon.toFixed(4)+"&start_date="+start+"&end_date="+end+"&daily=temperature_2m_max,temperature_2m_min,precipitation_sum,shortwave_radiation_sum,et0_fao_evapotranspiration,wind_speed_10m_max&timezone=auto";
  const r=await fetch(url,{signal:AbortSignal.timeout(12000)});
  if(!r.ok)throw new Error("Open-Meteo "+r.status);
  const j=await r.json();
  if(!j.daily)throw new Error("sin datos");
  return analizarClima(j.daily.time,j.daily.temperature_2m_max,j.daily.temperature_2m_min,j.daily.precipitation_sum,j.daily.shortwave_radiation_sum,j.daily.et0_fao_evapotranspiration,j.daily.wind_speed_10m_max);
}
    ...
    <div style={{fontFamily:"'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",
      ...
      <header style={{background:"#FFFFFF",color:P.tinta,padding:"10px 18px",display:"flex",
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <img src="/logo.png" alt="WineCheck" style={{height:42,width:"auto"}} />
