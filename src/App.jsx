        ...
        <>
          ...
          {clima&&(<>
            ...
            {precios&&precios.lista&&precios.lista.length>0&&(
              <Tarjeta titulo="Precio de mercado de la uva" estado="info">
                <div style={{fontSize:11,color:P.suave,fontStyle:"italic",marginBottom:6}}>Dato informativo de mercado (no afecta el veredicto de aptitud).</div><div style={{fontSize:12.5,color:P.tinta,marginBottom:6,lineHeight:1.45}}>Precio al que las viñas compran la uva {precios.region?"en la "+precios.region:"en esta región"} (para quien vende su producción a granel):</div>
      ...
      <div style={{display:"flex",flex:1,minHeight:0,flexDirection:esMovil?"column":"row"}}>
        <div style={{position:"relative",flexShrink:0,
          ...
          <MapContainer center={[-34.5,-71.2]} zoom={7} zoomControl={!esMovil} style={{height:"100%",width:"100%"}}>
            {capaBase==="calle"&&<TileLayer attribution='&copy; OpenStreetMap' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />}
            {capaBase==="satelite"&&<TileLayer attribution='&copy; Esri' url="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}" />}
            {capaBase==="topo"&&<TileLayer attribution='&copy; OpenTopoMap' url="https://a.tile.opentopomap.org/{z}/{x}/{y}.png" />}
          ...
          <div style={{position:"absolute",top:esMovil?58:"auto",bottom:esMovil?"auto":24,right:12,zIndex:1000,display:"flex",flexDirection:esMovil?"row":"column",gap:4,background:"rgba(255,255,255,.95)",borderRadius:8,padding:4,boxShadow:"0 2px 8px rgba(0,0,0,.2)"}}>
            {[["calle","Calle"],["satelite","Satélite"],["topo","Curvas"]].map(([k,t])=>(
