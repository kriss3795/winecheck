...
async function pedirAgua(lat,lon){
  ...
  // El servicio DGA usa wkid 5360 (SIRGAS-Chile). Mandamos el punto en 4326 con
  // inSR=4326 (ArcGIS reproyecta) y formato simple "lon,lat" que estos servidores
  // 10.21 manejan mejor que el JSON de geometria.
  async function consulta(){
    ...
    return await fetchGeo(base+"?"+params.toString(),9000);
  }
  try{
    const j=await consulta();
    if(j.error)throw 0;
...
export default function App(){
  ...
  async function evaluar(){
    ...
    try{
      const [c,a,suMulti]=await Promise.all([pedirClima(la,lo),pedirAgua(la,lo),pedirSuelosMultiples(puntos,[la,lo])]);
      setClima(c);setAgua(a);setSuelo(suMulti.principal);setSuelosMulti(suMulti.lista);setFase("resultado");
    }catch(e){setError(e.message||"error");setFase("resultado");}
  }
  function reiniciar(){setPuntos([]);setFase("mapa");setClima(null);setAgua(null);setSuelo(null);setSuelosMulti([]);setCentro(null);setError(null);setBib(false);}
