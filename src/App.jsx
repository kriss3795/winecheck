...
async function pedirClima(lat,lon){
  ...
  const base="https://archive-api.open-meteo.com/v1/archive?latitude="+lat.toFixed(4)+"&longitude="+lon.toFixed(4)+"&start_date="+start+"&end_date="+end+"&timezone=auto&daily=";
  // Intento 1: con todas las variables (incluye radiación, ET0, viento).
  ...
  // Intento 2 (respaldo): solo lo esencial, que SIEMPRE existe.
  const basico="temperature_2m_max,temperature_2m_min,precipitation_sum";
  let j=null;
  try{
    const r=await fetch(base+completo,{signal:AbortSignal.timeout(12000)});
    if(r.ok){ const jj=await r.json(); if(jj.daily) j=jj; }
  }catch(e){}
  if(!j){
    // Respaldo: si las variables extra fallaron, pedir solo lo esencial.
    const r=await fetch(base+basico,{signal:AbortSignal.timeout(12000)});
    if(!r.ok)throw new Error("Open-Meteo "+r.status);
    j=await r.json();
    if(!j.daily)throw new Error("sin datos");
  }
  const d=j.daily;
  return analizarClima(d.time,d.temperature_2m_max,d.temperature_2m_min,d.precipitation_sum,d.shortwave_radiation_sum,d.et0_fao_evapotranspiration,d.wind_speed_10m_max);
}
...
async function pedirCuerpoAgua(lat,lon){
  // 1) MAR: se detecta por elevación. El mar abierto no tiene polígonos OSM cercanos,
  //    pero está a nivel del mar o bajo él. Copernicus DEM da elevación negativa/~0.
  try{
    const el=await fetchGeo("https://api.open-meteo.com/v1/elevation?latitude="+lat.toFixed(5)+"&longitude="+lon.toFixed(5),8000);
    if(el&&el.elevation&&el.elevation.length){
      const e=el.elevation[0];
      if(e<=0){ return{enAgua:true,tipo:"Mar / océano",nombre:""}; }
    }
  }catch(e){}
  // 2) LAGOS / RÍOS / EMBALSES: Overpass con radio amplio (200 m) para cubrir
  //    lagos y cauces. En mar ya retornamos arriba.
  try{
    ...
    if(j&&j.elements&&j.elements.length>0){
      ...
      else if(t.natural==="water")tipo="Cuerpo de agua";
...
// ESCASEZ HÍDRICA: ¿la comuna/zona está bajo decreto de escasez hídrica vigente?
// Servicio DGA confirmado. Es un dato de contexto importante (no veta, pero informa).
async function pedirEscasez(lat,lon){
  ...
  }catch(e){return {escasez:false,error:true};}
}
...
async function pedirPreciosUva(regionNombre){
  try{
    ...
    return {lista, total:recs.length, region:regionNombre||null};
  }catch(e){return null;}
}
...
async function muestrearCapa(fn, sitios, clave){
  ...
  async function evaluar(){
    ...
    // allSettled: NUNCA se cuelga aunque una API falle o tarde. Cada capa se resuelve
    // sola; si una no responde, se ignora y el análisis continúa con el resto.
    const val=(r,def)=>r.status==="fulfilled"?r.value:def;
    const sitios=[...puntos,[la,lo]]; // todos los puntos marcados + centro
    try{
      const R=await Promise.allSettled([
        pedirClima(la,lo),                                          // 0 clima (centro; no varía en metros)
        muestrearCapa(pedirAgua,sitios,v=>v&&v.conectado?(v.titular||v.estado):null),        // 1 agua x sitios
        pedirSuelosMultiples(puntos,[la,lo]),                       // 2 suelo (ya multi)
        muestrearCapa(pedirHumedal,sitios,v=>v&&v.esHumedal?(v.nombre||v.tipo):"no"),          // 3 humedal x sitios
        muestrearCapa(pedirAreaProtegida,sitios,v=>v&&v.protegida?(v.nombre||v.tipo):"no"),    // 4 area x sitios
        pedirPendiente(puntos,[la,lo]),                             // 5 pendiente (ya usa todos)
        pedirRestriccionesExtra(la,lo),                            // 6 extra
        muestrearCapa(pedirCuerpoAgua,sitios,v=>v&&v.enAgua?(v.tipo+(v.nombre||"")):"no")      // 7 agua-cuerpo x sitios
      ]);
      const c=val(R[0],null);
      ...
      const aguaCuerpoList=val(R[7],[]);
      if(!c){ setError("No se pudo obtener el clima (revisa tu conexión)"); setFase("resultado"); return; }
  ...
  function reiniciar(){setPuntos([]);setFase("mapa");setClima(null);setAgua(null);setSuelo(null);setSuelosMulti([]);setHumedal(null);setSg(null);setProtegida(null);setPendiente(null);setExtras({activas:{},estado:{}});setCuerpoAgua(null);setPrecios(null);setEscasez(null);setAguaVar([]);setCentro(null);setError(null);setBib(false);}
          ...
          {clima&&(<>
            ...
            {precios&&precios.lista&&precios.lista.length>0&&(
              <Tarjeta titulo="Precio de mercado de la uva" estado="apto">
                <div style={{fontSize:12.5,color:P.tinta,marginBottom:6,lineHeight:1.45}}>Precio al que las viñas compran la uva {precios.region?"en la "+precios.region:"en esta región"} (para quien vende su producción a granel):</div>
                ...
                <div style={{fontSize:11.5,color:P.suave,marginTop:8,lineHeight:1.4}}>En Chile muchos productores venden la uva a granel a viñas que la vinifican; este es ese precio de referencia. Útil para estimar ingresos por hectárea.</div>
                <div style={{fontSize:11,color:"#A39C8E",marginTop:6}}>Fuente: ODEPA (Oficina de Estudios y Políticas Agrarias, Min. de Agricultura) — precios spot informados por los poderes compradores. Licencia CC-BY.</div>
              </Tarjeta>
            )}
            ...
            <details style={{marginTop:12,background:P.panel,border:"1px solid "+P.linea,borderRadius:8,padding:"10px 12px"}}>
              ...
              <div style={{fontSize:11.5,color:P.suave,lineHeight:1.5,marginTop:10}}>
                ...
                <div style={{marginBottom:4}}>
                  <div style={{fontWeight:700,color:P.tinta}}>Precios de uva — ODEPA</div>
                  <div><strong>Oficina de Estudios y Políticas Agrarias</strong> del Ministerio de Agricultura. Publica los precios oficiales de compra de uva (licencia CC-BY).</div>
                  <a href="https://datos.odepa.gob.cl/dataset/precios-uva-vinificacion" target="_blank" rel="noreferrer" style={{color:P.agua}}>datos.odepa.gob.cl ↗</a>
                </div>
